...
class Cano:
    ...
    def colidir(self,passaro):
        ...
        distacia_base = (self.x - passaro.x, self.pos_base - round(passaro.y))

        topo_ponto = passaro_mask.overlap(topo_mask, distacia_topo)
        base_ponto = passaro_mask.overlap(base_mask, distacia_base)
...
def gameOver(tela):
    ...
    texto_retry = FONT_PONTOS.render("Retry", True, (255,255,255))
    ...
    botao_react = pygame.Rect(botao_x,botao_largura, botao_y , botao_altura)
...
def desenhar_tela(tela, passaros, canos, chao, pontos):
    ...
    for cano in canos:
        cano.desenhar(tela)
    chao.desenhar(tela)
    text = FONT_PONTOS.render(f"Pontuação {pontos}", 1, (255, 255,255))
    ...
    while rodando:
        ...
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.KEYDOWN:
                if evento.key == pygame.K_SPACE or pygame.K_UP:
                    for passaro in passaros:
                        passaro.pular()
